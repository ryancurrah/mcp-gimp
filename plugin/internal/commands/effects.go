...
func applyVignette(p Params) (any, error) {
	...
	// GeglVignetteShape: 0 circle, 1 square, 2 diamond, 3 horizontal, 4 vertical.
	shapes := map[string]float64{
		"circle": 0, "square": 1, "diamond": 2,
		"horizontal": 3, "vertical": 4,
	}

	shape, ok := shapes[p.String("shape", "circle")]
	if !ok {
		return nil, fmt.Errorf("unknown vignette shape %q", p.String("shape", ""))
	}

	if err := runGeglFilter(drawable, "gegl:vignette", map[string]float64{
		"radius":   p.Float("radius", 1.5),
		"softness": p.Float("softness", 0.8),
		"shape":    shape,
	}); err != nil {
		return nil, err
	}
...
func exportWebOptimized(p Params) (any, error) {
	...
	if p.Bool("strip_metadata", true) {
		_ = run("gimp-image-set-metadata", gimpbridge.Args{"image": dup, "metadata-string": ""})
	}

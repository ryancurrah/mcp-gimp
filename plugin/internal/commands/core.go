...
func getGimpInfo(_ Params) (any, error) {
	...
	info["directories"] = map[string]any{"user": gimpbridge.UserDirectory()}
...
func baseTypeFor(mode string) (base string, alpha bool, err error) {
	switch mode {
	case "RGB":
	...
	case "RGBA":
		return "rgb", true, nil
	case "GRAY":
		return "gray", false, nil
	case "GRAYA":
		return "gray", true, nil
	case "INDEXED":
		return "indexed", false, nil
	default:
		return "", false, fmt.Errorf("unsupported color_mode %q", mode)
	}
}

// layerTypeFor names the GimpImageType a layer of an image with the given
// base type needs, such as rgba-image.
func layerTypeFor(base string, alpha bool) string {
	if alpha {
		return base + "a-image"
	}
...
func newCanvas(p Params) (any, error) {
	...
	lv, err := run1("gimp-layer-new", gimpbridge.Args{
		...
		"type": layerTypeFor(base, alpha || transparent), "opacity": 100.0, "mode": "normal",
	})
	if err != nil {
		return nil, err
	}
	...
	displayOpened := openDisplay(image) == nil
...
func openImage(p Params) (any, error) {
	...
	v, err := run1("gimp-file-load", gimpbridge.Args{"file": path, "run-mode": runNonInteractive})
	if err != nil {
		return nil, err
	}

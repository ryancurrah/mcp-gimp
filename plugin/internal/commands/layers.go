...
func createLayer(p Params) (any, error) {
	...
	// rgba-image so the layer can hold transparency.
	v, err := run1("gimp-layer-new", gimpbridge.Args{
		"image": image, "name": name, "width": width, "height": height,
		"type": "rgba-image", "opacity": p.Float("opacity", 100),
		"mode": p.String("blend_mode", "normal"),
	})
	if err != nil {
		return nil, err
	}
	...
	if fill := p.String("fill", ""); fill != "" {
		...
		if err := run("gimp-drawable-fill",
			gimpbridge.Args{"drawable": layer, "fill-type": "transparent"}); err != nil {
			return nil, err
		}
	}
...
func renameLayer(p Params) (any, error) {
	...
	_, layer, err := targetBy(p, "old_name")
	if err != nil {
		return nil, err
	}
...
func setLayerProperties(p Params) (any, error) {
	...
	if p.Has("blend_mode") {
		if err := run("gimp-layer-set-mode",
			gimpbridge.Args{"layer": layer, "mode": p.String("blend_mode", "normal")}); err != nil {
			return nil, err
		}
	}
...
func mergeVisibleLayers(p Params) (any, error) {
	...
	if err := run("gimp-image-merge-visible-layers",
		gimpbridge.Args{"image": image, "merge-type": "expand-as-necessary"}); err != nil {
		return nil, err
	}
...
func fillSelection(p Params) (any, error) {
	...
	if err := run("gimp-drawable-edit-fill", gimpbridge.Args{
		"drawable": drawable, "fill-type": p.String("fill_type", "foreground"),
	}); err != nil {
		return nil, err
	}

...
// ArgSpec is the exported description of a procedure argument.
type ArgSpec struct {
...
// Describe lists the arguments a PDB procedure declares.
//
// It must be called from the GIMP main thread; see [Do].
func Describe(name string) ([]ArgSpec, error) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	...
	n := int(C.mcp_procedure_arg_count(proc))
	out := make([]ArgSpec, 0, n)

	for i := range n {
		out = append(out, ArgSpec{
			Name:  C.GoString(C.mcp_procedure_arg_name(proc, C.int(i))),
			Type:  C.GoString(C.mcp_procedure_arg_type(proc, C.int(i))),
			Blurb: C.GoString(C.mcp_procedure_arg_blurb(proc, C.int(i))),
		})
	}
...
func bindOne(config *C.GimpProcedureConfig, spec argSpec, raw Value) error {
	...
	case string:
		...
		default:
			C.mcp_set_string(config, cname, cval)
		}
...
func ApplyGEGL(drawable ObjectID, operation string, settings map[string]float64) error {
	...
	if C.mcp_apply_gegl(C.gint32(drawable), cop, nameHead, valueHead,
		C.int(len(names)), &cerr) == 0 { //nolint:gocritic // cgo expansion
		msg := "unknown error"

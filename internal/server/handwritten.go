...
func addCheckServer(r *registrar) {
	schema, err := schemaFor[CheckServerInput](nil)
	if err != nil {
...
func addRestartServer(r *registrar) {
	schema, err := schemaFor[RestartServerInput](nil)
	if err != nil {
...
func addStateSnapshot(r *registrar) {
	schema, err := schemaFor[GetStateSnapshotInput](nil)
	if err != nil {
		...
		func(ctx context.Context, _ *mcp.CallToolRequest, in GetStateSnapshotInput) (*mcp.CallToolResult, any, error) {
			...
			if len(in.Region) > 0 {
				params.Region = &bitmapRegion{
					OriginX: regionInt(in.Region, "x", 0),
					OriginY: regionInt(in.Region, "y", 0),
					Width:   regionInt(in.Region, "width", maxSize),
					Height:  regionInt(in.Region, "height", maxSize),
				}
			}
...
// regionInt reads a numeric region field, falling back to fallback when the
// key is absent or not a number. JSON numbers decode as float64.
func regionInt(region map[string]any, key string, fallback int) int {
	v, ok := region[key]
	if !ok {
		return fallback
	}
...
func addCallAPI(r *registrar) {
	schema, err := schemaFor[CallAPIInput]([]string{"api_path"})
	if err != nil {

...
const drawPathDesc = `Stroke the outline of an SVG path on a layer, filling it first if asked.
...
- fill: Fill color for the path's interior, painted under the stroke, or
  "transparent" to erase it; omit to stroke only. An open path is closed for
  filling
...
Returns status dict, plus path_id when keep_path is true. With fill, also the
filled area's bounds {x, y, width, height}, and alpha_added: true when erasing
gave the layer an alpha channel.`
...
type DrawPathInput struct {
	...
	Fill       *string  `json:"fill" jsonschema:"Fill color for the path's interior, painted under the stroke, or \"transparent\" to erase it; omit to stroke only"`
...
const fillPathDesc = `Fill the interior of an SVG path with a solid color, and outline it if asked.

An open path is closed for filling, as SVG does, so the last point joins the
first. Use select_path for anything more than a flat fill.
...
- stroke_color: Outline color, drawn centred on the path; omit for no outline
...
Returns status dict with the filled area's bounds {x, y, width, height}, plus
path_id when keep_path is true and alpha_added: true when erasing gave the
layer an alpha channel.`
...
type FillPathInput struct {
	...
	StrokeColor *string  `json:"stroke_color" jsonschema:"Outline color, drawn centred on the path; omit for no outline"`
...
The selection composes with modify_selection, fill_selection, gradient_fill,
crop_to_selection and the rest. For a flat fill with an outline, fill_path
with stroke_color does it in one call. An open path is closed, as SVG does
when filling.
